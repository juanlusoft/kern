...
Toda decisi?n debe quedar ligada a un snapshot verificable del contexto de decisi?n, incluyendo una huella o referencia verificable de los atributos cr?ticos, pol?ticas y versiones evaluadas.

El snapshot debe incluir, cuando aplique, organizaci?n, identidades, delegaciones, concesiones, scopes, estado de revocaci?n, clasificaci?n, procedencia, taint, riesgo de integraci?n, aprobaci?n y versi?n de pol?tica.

Una decisi?n no puede reutilizarse sobre un snapshot de contexto, pol?tica o autorizaci?n distinto.
...
`limit` solo puede reducir l?mites cuantitativos, frecuencia, volumen, presupuesto, duraci?n, exposici?n de datos o alcance ya permitido. No puede cambiar organizaci?n, identidad, recurso objetivo, destinatario, acci?n, delegaci?n ni ampliar scope.

`require_approval` solo puede suspender una solicitud todav?a potencialmente autorizable. No puede convertir un deny terminal en una solicitud aprobable.

`defer` nunca es ejecutable y solo puede resolverse mediante una nueva evaluaci?n de Policy Engine con contexto verificable o terminar como deny efectivo.
...
Una decisi?n provisional no puede cruzar como autorizaci?n hacia Tool Engine, Knowledge Engine, channels, workflows, extensions, providers, runtimes ni sistemas externos.

Los componentes de enforcement solo pueden recibir una autorizaci?n ejecutable mediante Decision Binding final verificable conforme a RFC-0003.
...
Como m?nimo, la evidencia de una decisi?n relevante debe incluir identificadores y versiones de pol?ticas consideradas, referencia o huella del snapshot de contexto, resultado final, deny o defer aplicables, restricciones, obligaciones, transformaciones, aprobaci?n, Decision Binding final cuando exista, y eventos de revalidaci?n o invalidaci?n.

La auditor?a no debe almacenar por defecto el texto fuente completo de pol?ticas ni atributos sensibles innecesarios.
...
Los bundles de pol?ticas, pol?ticas compiladas, decisiones cacheadas, snapshots de contexto, resultados de evaluaci?n y artefactos de enforcement deben ser organization-scoped por defecto.

Cuando un artefacto de pol?tica se comparta como recurso de plataforma, debe existir aislamiento verificable, prop?sito expl?cito, ausencia de contexto empresarial reutilizable y auditor?a.

No puede reutilizarse impl?citamente una decisi?n, cach?, bundle compilado ni resultado de evaluaci?n de una organizaci?n para otra.
...
19. Una decisi?n solo es v?lida para el snapshot verificable de contexto y versiones de pol?tica sobre el que fue emitida.
20. Una decisi?n provisional no puede cruzar a un punto de enforcement como autorizaci?n.
21. `limit` nunca puede ampliar organizaci?n, identidad, recurso, acci?n, destinatario, delegaci?n o scope.
22. Los artefactos de pol?tica, cach?s y decisiones son organization-scoped por defecto y no se reutilizan impl?citamente entre organizaciones.
...
A?ade snapshot verificable de contexto y versiones de pol?tica, invalida decisiones ante cambios relevantes, proh?be que decisiones provisionales crucen a enforcement, precisa la sem?ntica de limit/require_approval/defer y extiende el aislamiento organizativo a artefactos y cach?s de pol?ticas.
