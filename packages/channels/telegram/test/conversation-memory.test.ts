...
test('conversation memory: recuerda los últimos turnos de un chat', () => {
  const mem = new ConversationMemory();
  const t = 1_000_000;
  mem.append('inst:chatA', 'user', 'haz un presupuesto para jlu.app', t);
  mem.append('inst:chatA', 'assistant', '¿qué quieres presupuestar?', t + 1);
  const recent = mem.recent('inst:chatA', t + 2);
  assert.equal(recent.length, 2);
  assert.deepEqual(recent[0], { role: 'user', content: 'haz un presupuesto para jlu.app' });
  assert.deepEqual(recent[1], { role: 'assistant', content: '¿qué quieres presupuestar?' });
});
...
test('conversation memory: acota a la ventana (últimos N)', () => {
  ...
  assert.deepEqual(recent.map((m) => m.content), ['msg 6', 'msg 7', 'msg 8', 'msg 9']);
});
...
test('conversation memory: olvida lo caducado', () => {
  ...
  // 2 s después → fuera de la ventana de 1 s
  assert.deepEqual(mem.recent('inst:chatA', t + 2000), []);
});

test('conversation memory: ignora contenido vacío', () => {
  const mem = new ConversationMemory();
  const t = 1_000_000;
