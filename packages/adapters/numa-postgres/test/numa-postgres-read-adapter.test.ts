...
test('HR builders keep name filters from short-circuiting when employee_id is null', () => {
  ...
  assert.deepEqual(punchDayStatement.values, ['org-acme', '2026-07-02', null, '%Ana Garc??a%', 25]);
  ...
  assert.deepEqual(leaveDaysStatement.values, ['org-acme', '2026-01-01', '2027-01-01', [34], null, '%Ana Garc??a%']);
  ...
  assert.deepEqual(worktimeStatement.values, ['org-acme', '2026-07-01', '2026-07-31', null, '%Ana Garc??a%']);
...
test('HR adapter forwards variable employee and group names without tying behavior to a single fixture', () => {
  ...
  assert.match(punchDayEmployeeName.toLowerCase(), /^ana garc/i);
  assert.equal(leaveDays.employee_name, 'Ana Garc??a');
  assert.equal(leaveBalance.employee_name, 'Juan Mag??n');
  assert.equal(worktime.employee_name, 'Ana Garc??a');
  ...
  assert.match(String(calls[0].values[3]).toLowerCase(), /^%ana garc/);
  assert.equal(calls[0].values[4], 25);
  assert.equal(calls[1].values[0], 'company-acme');
  assert.equal(calls[1].values[5], '%Ana Garc??a%');
  assert.equal(calls[2].values[0], 'company-acme');
  assert.equal(calls[2].values[5], '%Juan Mag??n%');
